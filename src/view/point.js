...
const createOffersListTemplate = (offers) => {
  return `<h4 class="visually-hidden">Offers:</h4>
        <ul class="event__selected-offers">
          ${offers.map(createOffersItemTemplate).join("")}
        </ul>`;
};
...
const createPointTemplate = ({
  ...
  return `<li class="trip-events__item">
                <div class="event">
                  <time class="event__date" datetime="${start.YYYYMMDD}">${
    start.MMMD
  }</time>
                  ...
                  <div class="event__schedule">
                    <p class="event__time">
                      <time class="event__start-time" datetime="${startTime}">${
    start.HHmm
  }</time>
                      —
                      <time class="event__end-time" datetime="${endTime}">${
    end.HHmm
  }</time>
                    </p>
                    <p class="event__duration">${start.init(end)}</p>
                  </div>
